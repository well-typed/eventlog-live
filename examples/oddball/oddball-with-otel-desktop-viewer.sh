...
# Find otel-desktop-viewer:
#
# 1. Use OTEL_DESKTOP_VIEWER if it is set.
# 2. Look for otel-desktop-viewer.
#
if [ "${OTEL_DESKTOP_VIEWER}" = "" ]; then
	if ! OTEL_DESKTOP_VIEWER="$(which "otel-desktop-viewer")"; then
		echo "Requires otel-desktop-viewer; no version found"
		exit 1
	fi
fi
...
resize -h 20%
focus left
screen -t 'otel-tui' sh -c 'eval "${OTEL_DESKTOP_VIEWER_CMD}"'
focus right
focus up
