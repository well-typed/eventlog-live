...
# Build fibber
echo "Build fibber"
cabal build fibber -v0 --project-file=cabal.project.profiling
...
"${FIBBER_BIN}" 44 +RTS -l -p --eventlog-flush-interval=1 -RTS >/dev/null &
FIBBER_PID=$!
...
echo "Start dumper"
sleep 5 && cabal run dumper -v0 -- --eventlog-socket "$GHC_EVENTLOG_SOCKET" "$@"
