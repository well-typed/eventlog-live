...
optsP = do
    ...
    optEkgPort <- optional $ O.option O.auto $
        O.long "ekg" <> O.metavar "PORT" <> O.help "EKG server port"
    optToFile <- optional $ O.strOption $
        O.long "write-to-file" <> O.metavar "PATH" <> O.help "Additionally write received eventlog to file"
